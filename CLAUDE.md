...
- Boundary validation via `_require_*` helpers, one per user-supplied value that reaches a URL or a jq program: `_require_build_number` / `_require_pr_id` / `_require_step_index` (numeric guards — the last two also close jq/URL injection surfaces, `_require_step_index` specifically prevents `$ENV.BB_TOKEN` exfiltration through the logs jq program), `_require_count` and `_require_pr_state` (query-param injection). Every value Python validates (`_is_positive_int` / `_validate_pr_id`) MUST have a bash `_require_*` sibling — parity gaps here are security bugs, not cosmetics.
