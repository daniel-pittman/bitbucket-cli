...
- `pr_create(title, source_branch?, destination_branch?, repo?, description?, close_source_branch?, reviewers?)` — `source_branch` auto-detects from the current git branch when empty; rejects detached-HEAD state. `reviewers` is a list of Bitbucket account UUIDs; resolve people to UUIDs with `members_list` (its `.user.uuid` is exactly this value), and never guess a UUID. `close_source_branch` defaults to `False`: branch deletion on merge is destructive, so it is opt-in, never automatic (the same stance `gh pr merge` takes with `--delete-branch`). Pass `True` to have the source branch deleted when the PR merges.
- `pr_update(pr_id, repo?, title?, description?, add_reviewers?, remove_reviewers?, drop_approvals?)` — update an OPEN PR's title and/or description via PUT to the same path `pr_show` GETs (Bitbucket Cloud has no PATCH for pull requests). Only the supplied fields change; the PUT merges them into the existing PR, preserving source/destination branches. `title` is two-way: omit (`null`) to leave unchanged, or pass a non-empty string (an empty/whitespace title is INVALID and surfaces as a `ValueError` envelope). `description` is three-way: omit (`null`) to leave unchanged, pass `""` to CLEAR it, or any string to set it (matches the bash `bb pr-update --description ""` clear). At least one of `title` / `description` must be supplied; an empty-body update is rejected at the boundary with no network IO. Requires pull-request write scope on the token. **Reviewers:** `add_reviewers` / `remove_reviewers` change who is asked to review an EXISTING PR (pr_create takes reviewers only at creation time). Bitbucket has no add-a-reviewer endpoint: the PUT REPLACES the whole `reviewers` array, so both options read the PR first and send the full resulting list — never try to add someone by passing a one-element list, which would unassign everyone else. Adding someone already on the PR is a no-op, not a duplicate. Resolve people to UUIDs with `members_list`. There is deliberately NO "set reviewers to exactly this" option: replace is what silently discards other people's approvals, and add/remove composes to the same result with each change stated. `drop_approvals` guards the destructive half — removing a reviewer who has ALREADY APPROVED discards that approval and re-adding them does not restore it, so it is refused unless you pass True. Leave it False unless the user asked for exactly that; removing a non-approver needs no opt-in.
- `pr_approve(pr_id, repo?)` / `pr_unapprove(pr_id, repo?)` — toggle approval.
- `pr_merge(pr_id, repo?, strategy?, close_source_branch?, message?)` — strategies: `merge_commit` (default), `squash`, `fast_forward`. `close_source_branch` defaults to `False` and is always sent explicitly, so it overrides whatever value the PR was stored with at creation (the merge API's value wins over the PR's): a PR created by an older `bb` or the Bitbucket UI with the box checked still keeps its source branch here unless you pass `True`. Branch deletion on merge is opt-in, never automatic (same stance as `gh pr merge --delete-branch`).
...
- `members_list(workspace?, count?)` — the members of a workspace via `GET /2.0/workspaces/{workspace}/members`. This is how you resolve a PERSON to the account UUID `pr_create(reviewers=[...])` requires: the PR API identifies reviewers ONLY by UUID, so without this there is no supported way to discover the value. Each record is a `workspace_membership` envelope whose `.user` carries `.uuid` (pass this to `reviewers`, braces included), `.display_name`, `.nickname`, `.account_id`. Match on `display_name` or `nickname`, then pass the matching `.user.uuid`. Nicknames are NOT unique in a workspace — when two members match, surface both and ask rather than guessing which person the user meant to add as a reviewer.
...
- `repo_show(repo?)` — single-repo metadata. The record carries `.project` (`.key` + `.name`), so this is how you answer "which project is this repo in?" — and it does NOT require `read:project:bitbucket` (verified against a token lacking that scope), unlike `projects_list`. Use it to confirm a `repo_update(project=...)` landed on the intended key: project keys are not reliably the obvious abbreviation of the project name, and writing a wrong-but-valid key otherwise looks identical to writing the right one.
...
- `repo_update(repo?, project?, description?)` — update an existing repo via PUT to the same path `repo_show` GETs. The dominant use is moving a repo between projects: pass `project` with the target project KEY (`repo_create` takes a project on creation but nothing else could change it afterward). At least one of `project` / `description` must be supplied; an empty-body update is rejected at the boundary (no network IO). `project` is two-way: omit it (`null`) to leave it unchanged, or pass a KEY to move the repo. An empty/whitespace `project` is INVALID (there is no "clear the project" — every repo has one) and surfaces as a `ValueError` error envelope rather than being silently dropped, so a `project="" , description="..."` call can't quietly skip the move and report success. `description` is three-way: omit it (`null`) to leave it unchanged, pass `""` to intentionally CLEAR it, or any other string to set it (matches the bash `bb repo-update --description ""` clear). Accepts the repo in the same shapes as every other repo command (`"slug"`, `"ws/slug"`, or `""` auto-detect). Returns the updated record under `info` plus the resolved `project` key. Requires `admin:repository:bitbucket` scope (same as `repo_create`); `write:repository:bitbucket` alone returns 403, body names the missing scope under `error.detail.required`.
...
- `vars_delete(key, repo?, scope?, environment?)` — DELETE a pipeline variable by key at the chosen scope. Resolves the key to its UUID (walks all pages, same as `vars_set`), then DELETEs `.../variables/{uuid}`. A key absent at the scope returns a clean `BBOpNotFound` in the envelope with NO delete issued, so a typo can't silently no-op. `scope` / `environment` work exactly as for `vars_set`. Returns `{key, scope, environment, action: "deleted"}`. Requires `admin:pipeline:bitbucket` scope (same as `vars_set`); `write:pipeline:bitbucket` alone returns 403. DESTRUCTIVE and effectively irreversible for a secured value, so treat it as PROPOSE-FIRST when invoked ad hoc (see operating principle 5).
...
When invoking `pr_merge`, `pr_decline`, `pipeline_stop`, `pr_unapprove`, `vars_delete`, or `environment_delete` from a delegated context, surface the what / why / new-state to the user first when there's any ambiguity. `vars_delete` especially: a deleted secured value can't be recovered, so name the key + scope and confirm before firing. `pr_approve` and `pr_comment_add` are reversible enough to fire without a propose step in normal flow.
