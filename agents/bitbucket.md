...
- `pr_create(title, source_branch?, destination_branch?, repo?, description?, close_source_branch?, reviewers?)` — `source_branch` auto-detects from the current git branch when empty; rejects detached-HEAD state. `reviewers` is a list of Bitbucket account UUIDs. `close_source_branch` defaults to auto: True (delete on merge) for short-lived branches, False when the source is long-lived (`main`/`master`/`develop`/`dev`/`release/*`) — a promote PR never deletes its source trunk. Pass an explicit bool to override either way.
...
- `pr_merge(pr_id, repo?, strategy?, close_source_branch?, message?)` — strategies: `merge_commit` (default), `squash`, `fast_forward`. `close_source_branch` defaults to auto: the tool looks up the PR's source branch and keeps it (False) when it is long-lived (`main`/`master`/`develop`/`dev`/`release/*`), overriding whatever the PR was stored with (the merge API's value wins); short-lived sources keep the delete-on-merge default. An unreadable source branch fails safe to keep. Pass an explicit bool to force either behaviour.
