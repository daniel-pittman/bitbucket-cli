...
- `pr_create(title, source_branch?, destination_branch?, repo?, description?, close_source_branch?, reviewers?)` — `source_branch` auto-detects from the current git branch when empty; rejects detached-HEAD state. `reviewers` is a list of Bitbucket account UUIDs. `close_source_branch` defaults to `False`: branch deletion on merge is destructive, so it is opt-in, never automatic (the same stance `gh pr merge` takes with `--delete-branch`). Pass `True` to have the source branch deleted when the PR merges.
